...
const reviewsUpdateOne = (req, res) => {
    ...
    Loc
        ...
        .exec((err, location) =>{
            ...
            if(location.reviews && location.reviews.length > 0)
            {
                const thisReview = location.reviews.id(req.params.reviewid); //Finds the subdocument
                if(!thisReview)
                {
                    res
                        .status(404)
                    ...
                    // Make changes to the subdocument from the data supplied
                    thisReview.author = req.body.author;
                    thisReview.rating = req.body.rating;
                    thisReview.reviewText = req.body.reviewText;
                    location.save((err, location) =>{ //Save the parent document
                        if(err)
                        {
                            res
                                .status(404)
